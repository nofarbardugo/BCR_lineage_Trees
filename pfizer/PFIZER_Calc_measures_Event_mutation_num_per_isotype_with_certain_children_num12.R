  ...
  while(p <=sunsNum)
  {
    if(suns[p]!= "VJ_1")
    {
      ...
      if(type == getIsotypeName(suns[p])) 
      {
        ...
        # not a leaf - calc sun node details and add it to the count
        else
        {
...
write.csv(df.allIsotypeDetails,file = "PFIZER_mutation_num_per_isotype_with_certain_children_numberWithInternal.csv")
