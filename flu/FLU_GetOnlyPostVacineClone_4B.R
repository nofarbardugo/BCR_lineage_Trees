  ...
  for(i in 1:length(dirs))
  {
    ...
    if(file.exists(filePath))
    {
      ...
      if (nrow(df.edges)!=0)
      {
        ...
        isotypesNames <- c(isotypesNames.df[grep('RL016', isotypesNames.df$TimePoint),"head"], 
                           isotypesNames.df[grep('RL017', isotypesNames.df$TimePoint),"head"],
                           isotypesNames.df[grep('RL018', isotypesNames.df$TimePoint),"head"],
                           isotypesNames.df[grep('RL019', isotypesNames.df$TimePoint),"head"], 
                           isotypesNames.df[grep('RL020', isotypesNames.df$TimePoint),"head"],
                           isotypesNames.df[grep('RL021', isotypesNames.df$TimePoint),"head"],
                           isotypesNames.df[grep('RL022', isotypesNames.df$TimePoint),"head"])
        ...
        write.table(df.edgesCut, file = paste0(dirPath, "_edgesCut_postVacine.tab"), quote=F, sep='\t', col.names=T, row.names=F, append = F)
        print(paste0(dirPath, "_edgesCut_postVacine.tab"))
      }
