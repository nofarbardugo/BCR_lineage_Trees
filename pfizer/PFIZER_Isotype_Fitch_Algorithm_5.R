...
source.dir <-"/home/bardugn1/pfizer" #pep 4
setwd(source.dir)
...
{
  return (switch(as.character(isotypeNumber), "6"="naive_IGHM", "5"="naive_IGHD", "4"="IGHM", "3"="IGHG", "2"="IGHE", "1"="IGHA"))
}
  ...
  #   reutrn the isotypes father 
{
  ...
  # in case thier is only IGA and IGE their father is IGD 
  ...
  # Returns:the node isotype name
{
  ...
  if(length(suns)==0)
  {
    ...
    {
      isotypeName <- substr(fullName,makafNumber[[1]][1]+1,makafNumber[[1]][3]-1)
    }
    else
    {
      isotypeName <- substr(fullName,makafNumber[[1]][1]+1,makafNumber[[1]][2]-1)
    }
  ...
  for(i in 1:length(dirs))
  {
    ...
    if(file.exists(filePath))
    {
      ...
      if(!is.null(df.edges))
      {
        ...
        fd.TotalLeavesType<- data.frame(treeNUM=c() ,IGHA = c(),IGHE =c(),IGHG = c()
                                        ,IGHM = c(), naive_IGHD = c(),naive_IGHM = c(),sum_Leaves =c(),type_num = c())
        p<-1
