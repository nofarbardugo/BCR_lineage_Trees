...
ChangeIsotypeToNumber <- function(isotypeCharacter)
{
  ...
  return (switch(isotypeCharacter, "naive_IGHM"=6, "naive_IGHD"=5, "IGHM"=4, "IGHG"=3, "IGHE"=2, "IGHA"=1, NA))
}
...
fd.TotalLeavesForALL<- data.frame(treeNUM=c() ,IGHA = c(),IGHE =c(),IGHG = c()
...
for(k in 1:length(patient.dirs))
{
  ...
  # data fream that save details about each tree leaves - just for get details
  fd.TotalLeavesForPatient<- data.frame(treeNUM=c() ,IGHA = c(),IGHE =c(),IGHG = c()
                                        ,IGHM = c(), naive_IGHD = c(),naive_IGHM = c(),sum_Leaves =c(),type_num = c())
  ...
  for(i in 1:length(dirs))
  {
    ...
    if(file.exists(filePath))
    {
      ...
      if(!is.null(df.edges))
      {
        ...
        fd.TotalLeavesTypeForClone<- data.frame(treeNUM=c() ,IGHA = c(),IGHE =c(),IGHG = c()
                                                ,IGHM = c(), naive_IGHD = c(),naive_IGHM = c(),sum_Leaves =c(),type_num = c())
        p<-1
        while(p <=length(roots))
        {
          
          fd.leavesType <- data.frame(treeNUM=c(paste(dirs[i],roots[p],sep = '_')) ,IGHA = c(0),IGHE =c(0),IGHG = c(0)
                                      ,IGHM = c(0), naive_IGHD = c(0),naive_IGHM = c(0),sum_Leaves =c(0),type_num = c(0))
          
          GoOverTree(roots[p])
          
          # calc total leaves num 
          fd.leavesType[1,"sum_Leaves"] <- fd.leavesType[1,"IGHA"]+fd.leavesType[1,"IGHE"] +
            fd.leavesType[1,"IGHG"] + fd.leavesType[1,"IGHM"] +
        ...
        write.csv(fd.TotalLeavesTypeForClone, file = paste0(dirPath,"_totalLeavesType_combineAB.csv"))
...
# save all petient togther
write.csv(fd.TotalLeavesForALL, file = paste0(source.dir,"/ALL_totalLeavesType_combineAB.csv"))

a <- fd.TotalLeavesForALL[fd.TotalLeavesForALL$type_num>1&fd.TotalLeavesForALL$sum_Leaves>10,]