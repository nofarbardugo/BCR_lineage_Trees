...
fd.TotalLeavesForALL<- data.frame(treeNUM=c() ,IGHA = c(),IGHE =c(),"IGHG.1" = c(),"IGHG.2" = c()
                            ,IGHM = c(), naive_IGHD = c(),naive_IGHM = c(),sum_Leaves =c(),type_num = c(),patient_name = c())
...
for(k in 1:length(patient.dirs))
{
  ...
  fd.TotalLeavesForPatient$patient_name <- patient.dirs[k]
